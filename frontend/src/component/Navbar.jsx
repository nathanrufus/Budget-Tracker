...
function Navbar() {
  ...
  const authLinks = [
    ...
    { name: "Logout", path: "/signin", onClick: () => { localStorage.removeItem("token"); window.location.href = "/signin"; } }
  ];
    ...
    <nav className="bg-blue-100 p-4 fixed top-0 left-0 right-0 w-full z-50">
        ...
        <ul className="hidden md:flex space-x-8 text-gray-800">
          {navItems.map(({ name, path }) => (
            <li key={name} className="relative group">
              <Link to={path} className="hover:text-purple-600 transition duration-300">
                {name}
              </Link>
          ...
          {isAuthenticated && authLinks.map(({ name, path, onClick }) => (
            <li key={name} className="relative group">
              <Link
                to={path}
      ...
      {menuOpen && (
        <ul className="absolute top-full left-0 w-full bg-white shadow-lg p-4 space-y-4 text-gray-800 text-center rounded">
          ...
          {isAuthenticated ? (
            authLinks.map(({ name, path, onClick }) => (
              <li key={name}>
                <Link
                  to={path}
                  className="block hover:text-purple-600"
